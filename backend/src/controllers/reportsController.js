...
//     try {
//         const { range, agentId, startDate, endDate } = req.query;
...
//         if (dateRange) {
//             dateFilter.calledAt = dateRange;
//         }

//         // Build agent filter (admin can see all, others see only themselves)
//         let agentFilter = {};
...
//         const monthlySummary = await getMonthlySummary(finalFilter, userRole, agentId);

//         // ── Weekly Trend (last 7 weeks) ───────────────────────
//         const weeklyTrend = await getWeeklyTrend(finalFilter);

//         // ── Call Distribution (Pie chart data) ────────────────
//         const callDistribution = await getCallDistribution(finalFilter);

//         // ── Agent Performance ─────────────────────────────────
//         let agentPerformance = [];

//         if (["admin", "super_admin", "manager"].includes(userRole)) {
//             agentPerformance = await getAgentPerformance(finalFilter, range);
//         } else {
//             // Single agent view
//             agentPerformance = await getSingleAgentPerformance(userId, finalFilter);
//         }

//         // ── Summary Cards ─────────────────────────────────────
//         const summaryCards = await getSummaryCards(finalFilter);
...
//             callDistribution,
//             agentPerformance,
//             filters: { range, startDate, endDate, agentId }
//         });
...
// async function getMonthlySummary(filter, userRole, agentId) {
...
// async function getAgentPerformance(filter, range) {
...
// async function getSingleAgentPerformance(agentId, filter) {
...
//         { title: "Connected Calls", value: connected.toLocaleString(), change: `+${Math.round((connected / total) * 100)}%`, up: true },
//         { title: "Missed Calls", value: missed.toLocaleString(), change: `-${Math.round((missed / total) * 100)}%`, up: false },
...
exports.getReports = async (req, res) => {
    try {
        ...
        // ── Monthly Summary (last 6 months) ───────────────────
        const monthlySummary = await getMonthlySummary(finalFilter);
        ...
        if (["admin", "super_admin", "manager"].includes(userRole)) {
            agentPerformance = await getAgentPerformance(finalFilter);
        } else {
            // Single agent view - pass req.user
            agentPerformance = await getSingleAgentPerformance(userId, finalFilter, req.user);
        }
